...
\begin{SaveDefinition}[key=Q-learning, title ={Q-learning Algorithm}]
...
% \LinesNumbered
	...
	\For{each week $n$}{
		...
		$R_n = \underbrace{s \min (D_n,X_n)}_{\text{profit from sales}} - \underbrace{d(a_n>0)}_{\text{delivery cost}} - \underbrace{cK_n}_{\text{damage cost}}$ \quad (reward/profit for the week)
		
		$\displaystyle Q(X_n,a_n) \leftarrow (1-\alpha_n)Q(X_n,a_n) + \alpha_n\left(R_n + \gamma \max_{a} Q(X_{n+1},a)\right)$
	} 
\end{algorithm}
