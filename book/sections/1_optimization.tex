...
\begin{itemize}
	\item Define $p(x) = $ price of a \textsl{plastic} chair so that $x$ \textsl{plastic} chairs are sold per day.
\end{itemize}

%\paragraph{Note:} We are trying to obtain an \textsl{(ideal)} mathematical model.

\begin{parts}
	\item As more \textsl{plastic} chairs are sold in the market: $x \to \infty$, what do you expect will happen to the selling price?
	
%	$$ p \to \text{Manufacturing Cost}$$
	
	\item As \textsl{plastic} chairs become scarce: $x \to 0^+$, what happens to the price?
	
%	$$p \to \infty$$
	
	\item What family of functions for $p(x)$ satisfies both these conditions?
...
\setcounter{partsitem}{3}
	...
	\item On the graph on the right, sketch the points $x_2$ and $x_3$.
...
%		\item Download the file \href{https://raw.githubusercontent.com/bigfatbernie/IBLMathModeling/main/book/python/chairs_newton.ipynb}{\tt chairs\_newton.ipynb} and import it into the Jupyter Notebook
...
\hspace{-2em}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item $a_3 = 0.0250 $
		\end{itemize}
		\item The total load is 952 MW
	\end{itemize}


	\item Use Lagrange multipliers to set up the solution as a linear system and solve it. You can use the \texttt{numpy} function \texttt{linalg.solve}.
...
\textbf{Report the results.}
	...
	\item A customer installs solar panels at their home and wants to pump electricity back to the grid. What should the energy company pay the customer? \\
...
\setcounter{partsitem}{2}
	...
	\item Define:
		...
		\item If we increase the total load ($X$) by 1 MW, it would approximately increase the total cost of operating the three power plants by \$15/h. Observe that the average cost at 952 MW is \$8 / MW$\cdot$h, so this is almost twice the average cost -- this is not in the optimal range at all.

		\item Similarly to the previous question, if we reduce the total load ($X$) by 1 MW, it would approximately reduce the total cost of operating the three power plants by \$15/h.
...
Suppose a manufacturing company receives an order for $B$ units to be delivered at time $T$, e.g. Sobeys has placed an order for $B = 5$ pallets of Chapman's vanilla ice-cream for a promotion starting in $T = 4$ days.
...
\begin{itemize}
	\item Inventory Cost $\displaystyle \begin{aligned}[t]
			& = \sum_{n=1}^N \Delta x_n (T-t_n) c_2 \\
			& = \sum_{i=1}^N \sum_{j=1}^{n-1} \Delta x_j c_2
		\end{aligned}
		$
...
\begin{itemize}
	\item Total Cost $\displaystyle = \sum_{n=1}^N \bigg[ \frac{\Delta x_n^2}{\Delta t} c_1 + \Delta x_n (T- n \Delta t ) c_2 \bigg]$
\end{itemize}
...
\begin{parts}
	...
	\item Let $\displaystyle g(\varepsilon) = \int_{t_0}^{t_1} F\big( t, y(t), y'(t) \big) ~dt$. Expand the formula for $g(\varepsilon)$ and find $g'(\varepsilon)$ (remember the chain rule from multivariable calculus).
