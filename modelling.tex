...
\begin{enumerate}
	...
	\item \eqref{LM} $\Leftrightarrow \nabla f(x^\star) \in {\rm span}\{\nabla g_1(x), \ldots, \nabla g_k(x)\}$.
	\item The ``optimal'' values for $\lambda_1, \ldots, \lambda_k$ give important insights on the problem -- don't ignore them!
\end{enumerate}
\end{definition}
...
\bigskip
	...
	\end{enumerate}
	\begin{solution}
		 $x^\star=y^\star= \frac{\sqrt{c}}{\sqrt{2}}$ \quad and \quad $\lambda^\star = \frac{1}{\sqrt{2c}}$
	...
	\begin{solution}
		\[
		\frac{\partial f(x^\star,y^\star)}{\partial c}
			= \frac{\sqrt{2}}{2\sqrt{c}} = \lambda^\star
		\]
	\end{solution}
	...
	\begin{solution}
		This means that if the constraint increased from $1$ to $1 + \Delta = 1.1$, 	then we would expect the maximum to increase by approximately $\Delta \lambda^\star = \frac{\Delta}{\sqrt{2}} \approx 0.07$.
	\end{solution}
...
\begin{problem}%[Wood vs Aluminum]
...
\setcounter{partsitem}{3}
	...
	\begin{enumerate}
		...
		\item Download the file \url{https://github.com/bigfatbernie/IBLMathModeling/blob/main/python/chairs_newton.ipynb} and import it into the Jupyter Notebook
...
\setcounter{partsitem}{5}
	...
	\begin{enumerate}
		\item Go to \url{https://utoronto.syzygy.ca/jupyter}
		\item Download the file \url{https://github.com/bigfatbernie/IBLMathModeling/blob/main/python/chairs_fmin.ipynb} and import it into Jupyter Notebook
...
\textbf{Sensitivity}. To compute $p^\star$, you can use \url{https://github.com/bigfatbernie/IBLMathModeling/blob/main/python/chairs_sensitivity.ipynb}

\begin{parts}
	\item How sensitive is the profit to the parameter $c = 10$ (the production cost of the aluminum chair)
		\[ S(p^\star, c) \approx \frac{p^\star(c+h) - p^\star(c)}{h} \frac{c}{p^\star(c)}?
		\]

	\item How sensitive is the profit to the parameter $b = 0.4$ (the exponent of $y$ in the selling price of the aluminum chair)
		\[ S(p^\star, b) \approx \frac{p^\star(b+h) - p^\star(b)}{h} \frac{b}{p^\star(b)}?
		\]
		
\end{parts}

% We already calculated all the terms in these expressions for S except the bold faced ones. Those are the only ones we need to calculate.
